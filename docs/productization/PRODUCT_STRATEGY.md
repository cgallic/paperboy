...
**Product:** Hosted self-serve daily or weekly delivery at `https://newpaperboy.com/`. The
flow is operationally deployed, but paid conversion remains unproven until a real live
charge, signed webhook, renewal state, portal, and cancellation are verified end to end.
...
Production billing must not be described as proven until a recurring `$5/month` Stripe
price is selected, these values are configured, and a real authorized transaction completes:
